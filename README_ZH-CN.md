...
1. 先执行 `yarn build`（开发阶段可用 `yarn dev`）并按上面的步骤把扩展加载到 Chrome/Edge。
2. 进入 **全能模式**（多机器人面板），至少保持两个机器人在线。
3. 在布局切换按钮旁打开 **Auto-Summarize** 开关。
4. 提交一个问题，等待所有机器人回复完成（或等待 30 秒超时自动收集可用回复）。
5. 确认第一个机器人会自动发送一条“裁判式”总结，归纳并指出其它回复的差异。
6. 关闭 Auto-Summarize 再发一条问题，确认普通多机器人轮次仍然按原有方式工作。
